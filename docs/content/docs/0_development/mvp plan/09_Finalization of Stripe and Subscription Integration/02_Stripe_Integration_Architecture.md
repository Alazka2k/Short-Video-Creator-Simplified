...
### 1. Frontend Integration ❌ Nothing done yet
...
3.  **UI Components**:
    *   `PlanSelection` & `TokenPackagePurchase` Components: Display product offerings and integrate `CheckoutButton`.
    *   `UserDashboard` & `SubscriptionManagement`: Display current subscription status and token balance (data sourced from backend, synchronized via webhooks) and token usage with information about current plan and billing period, plan tokens, tokens used, Additional (one time tokens) and a button for purchase more tokens. Integrate `CustomerPortalButton`. Possibility to view full history of token consumption. 

### 2. Backend Integration 🔄 Needs review and enhancement after auth flow is changed
...
### 3. Batch Processing 🔄 Needs review and enhancement what and how it is still needed
