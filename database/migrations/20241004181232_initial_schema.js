// File: database/migrations/YYYYMMDDHHMMSS_initial_schema.js
...
exports.up = function (knex) {
  return knex.schema
    ...
    // 2. Jobs Table (now using UUID)
    .createTable('jobs', function (table) {
      table.uuid('job_id').primary().defaultTo(knex.raw('uuid_generate_v4()'));
    ...
    .createTable('user_subscriptions', function (table) {
      ...
      table.integer('user_id').unsigned().references('user_id').inTable('users').onDelete('CASCADE');
      table.integer('plan_id').unsigned().references('plan_id').inTable('plans');
      table.date('start_date').notNullable();
      table.date('end_date');
      table.string('status', 20).notNullable().checkIn(['active', 'canceled']);
      table.timestamps(true, true);
    })

    .createTable('tokens', function (table) {
      table.increments('token_id').primary();
      table.integer('user_id').unsigned().unique().references('user_id').inTable('users').onDelete('CASCADE');
    ...
    .createTable('token_transactions', function (table) {
      ...
      table.integer('user_id').unsigned().references('user_id').inTable('users').onDelete('CASCADE');
      table.string('transaction_type', 20).notNullable().checkIn(['allocation', 'deduction', 'purchase']);
      table.integer('amount').notNullable();
      table.timestamp('transaction_date').defaultTo(knex.fn.now());
      table.uuid('job_id').references('job_id').inTable('jobs');
    })
    ...
    .createTable('payments', function (table) {
      ...
      table.string('status', 20).notNullable().checkIn(['pending', 'completed', 'failed']);
    })
    ...
    .createTable('llm_outputs', function (table) {
      ...
      table.integer('llm_input_id').unsigned().references('llm_input_id').inTable('llm_inputs').onDelete('CASCADE');
      table.uuid('job_id').references('job_id').inTable('jobs').onDelete('CASCADE');
    ...
    .createTable('llm_scenes', function (table) {
      ...
      table.integer('llm_output_id').unsigned().references('llm_output_id').inTable('llm_outputs').onDelete('CASCADE');
    ...
    .createTable('image_outputs', function (table) {
      ...
      table.uuid('job_id').references('job_id').inTable('jobs').onDelete('CASCADE');
      table.integer('scene_id').unsigned().references('scene_id').inTable('llm_scenes').onDelete('CASCADE');
