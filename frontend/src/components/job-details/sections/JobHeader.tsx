...
interface JobHeaderProps {
  ...
  focus?: string;
  status: 'in_progress' | 'failed' | 'queued' | 'completed' | string;
}
...
export function JobHeader({
  ...
  const getDescription = () => {
    switch(status) {
      case 'in_progress':
      case 'queued':
        return `Created ${formatDistanceToNow(new Date(created_at), { addSuffix: true })}.`;
      case 'failed':
        return `Created ${formatDistanceToNow(new Date(created_at), { addSuffix: true })}. The job could not be completed.`;
      case 'completed':
      default:
        return description;
    }
  ...
  const renderStatusIcon = () => {
    switch(status) {
      case 'in_progress':
      case 'queued':
        return null; // Spinner is in JobStatusView
      case 'failed':
        return <AlertTriangle className="w-5 h-5 mr-2 text-destructive" />;
      default:
        return null;
    }
  }
  
  const showExpandableDetails = status !== 'failed';
  ...
  return (
    <div className="rounded-xl bg-gradient-to-r from-violet-500/20 to-purple-500/20 p-[1px]">
      <div className="space-y-4 bg-card rounded-xl p-6">
        ...
        {/* Details section, shown for in-progress and completed jobs */}
        {showExpandableDetails && (
          <>
            {/* Hashtags, only shown when completed and available */}
            {status === 'completed' && hashtag && (
            ...
            >
              <span className="mr-2">{showDetails ? 'Hide' : 'Show'} Initial Prompt & Parameters</span>
