...
• 1.2. Content recreation (call to each job with job id, update the job database entry itself after response is received, to always show the latest content), but versionize the job entry itself
...
• 1.3 Content download functionality
...
• Initial Prompt
• Scene amount
• Since it is a bento grid, when is the size changing?
